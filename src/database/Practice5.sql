...
select 제품명,카테고리명 from product inner join pcategory on product.카테고리번호_fk = pcategory.카테고리번호_pk;
-- [문제 2] '노트북' 카테고리에 속하는 모든 제품의 제품명과 제품가격을 조회하세요. (조회결과: 2개 레코드)
select 제품명,제품가격 from  product inner join pcategory on product.카테고리번호_fk = pcategory.카테고리번호_pk where 카테고리명="노트북";
...
select 재고등록날짜 ,재고수량 from product inner join stock on product.제품번호_pk = stock.제품번호_fk where 제품명="그램 15인치";
...
SELECT 제품명 FROM product LEFT JOIN stock ON product.제품번호_pk = stock.제품번호_fk WHERE stock.제품번호_fk IS NULL;
-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드)
SELECT 카테고리명, SUM(재고수량) AS 총재고수량 FROM product 
INNER JOIN pcategory ON product.카테고리번호_fk = pcategory.카테고리번호_pk 
INNER JOIN stock ON product.제품번호_pk = stock.제품번호_fk GROUP BY 카테고리명;
-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 레코드)
select 제품명, sum(재고수량) as 총재고수량 from product inner join stock on product.제품번호_pk = stock.제품번호_fk GROUP BY 제품명 order by 총재고수량 desc;