...
select customer, sum(order_qty*price) AS 총구매금액 from orders join books on orders.book_id = books.book_id group by customer;
...
select genre , sum(order_qty*price) AS 총매출 from orders join books on orders.book_id = books.book_id group by genre;
-- 출력: genre, 총매출
...
select customer , sum(order_qty*price) AS 총구매금액 from orders join books on orders.book_id = books.book_id group by customer having 총구매금액 >=100000;
-- 출력: customer, 총구매금액
